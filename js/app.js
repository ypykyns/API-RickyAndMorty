...
const imprimirDatos = (datos,e) => {
    ...
    datos.results.forEach(pjs => { //despues de iterar pjs es un objeto con todas las propiedades
        ...
        p.addEventListener('click', () => {
            ...
            status.innerHTML = `O personagem se encontra: ${pjs.status}`
            species.innerHTML = `O personagem é da espécie: ${pjs.species}`
            origen.innerHTML = `Sua origem é: ${pjs.origin.name}`
        })
    });
};
